...
# Install actual Ruby
RUN ruby-install ruby 2.1.7 -- --disable-install-doc

# Add Ruby binaries to $PATH
ENV PATH $PATH:/opt/rubies/ruby-2.1.7/bin
RUN echo 'export PATH="$PATH:/opt/rubies/ruby-2.1.7/bin"' > /etc/profile.d/ruby.sh
RUN chmod a+x /etc/profile.d/ruby.sh
RUN echo '\nsource /etc/profile.d/ruby.sh' >> /etc/bash.bashrc

# Adjust user gem settings
RUN echo 'if (( $UID != 0 )); then\n\texport GEM_HOME="$HOME/.gems/2.1.7"\n\texport PATH="$PATH:$GEM_HOME/bin"\nfi' >> /etc/profile.d/ruby.sh

# Never install Ruby docs
RUN mkdir /opt/rubies/ruby-2.1.7/etc
RUN echo "gem: --no-document" > /opt/rubies/ruby-2.1.7/etc/gemrc
